...
note:
1 使用如下四个模型计算likelihood, 之后再运行

2 降低条件至-0.25，

3 动量0.5

4 guass kernel size = 3
...
# 获得人脸识别的model(backbone), 该模型输出512维向量
...
def noise_iter(model_pool, origin_img, target_img, mask, gaussian_blur, device):
    ...
    while True:    
        index += 1
        tmp_noise, socre1, socre2, loss1_v = iter_step(tmp_noise, origin_img, target_img, mask, gaussian_blur, model_pool, index, loss1_v, momentum, lr=learn_rate)
...
def cal_adv(origin_name, target_name, mask_name, model_pool, gaussian_blur, device):
    ...
    while True:
        ...
        if socre < -0.4:
            socre = -0.4
        socres = 0.5 * socre + 0.5 * socres
        i += 1
        if i > 300:
            f = open('hard.txt', 'a')
            f.write(origin_name + ';' + target_name + ';' + str(socres) + '\n')
            f.close()
            print('origin img is %s, target img is %s, iter %d, socre is %0.3f'
                % (origin_name.split('/')[1], target_name.split('/')[1], i, socres))
            break

        if socres < break_threshold:
            print('origin img is %s, target img is %s, iter %d, socre is %0.3f'
                % (origin_name.split('/')[1], target_name.split('/')[1], i, socres))
            break;
...
# 使用一个显卡生成对抗样本, 一个显卡可以选择使用两个进程同时运行
...
if __name__ == '__main__':
    ...
    learn_rate = 8
    print('learn rate is ' + str(learn_rate))
